        ...
        <div class="card">
          <div class="card-body">
            <form action="{{ route('promo.update', [$promo->id]) }}" method="POST">
              ...
              <div class="row">
                <div class="col-3">
                  <div class="form-group">
                    ...
                    <input type="text" name="nama" id="nama" class="form-control" placeholder="Masukkan nama promo" required value="{{ $promo->nama }}">
                  </div>
                </div>
                <div class="col-3">
                  <div class="form-group">
                    ...
                    <div class="d-flex justify-content-center align-items-center">
                      <div class="mr-1" style="width: 60%;">
                        <input type="text" name="diskon" id="diskon" class="form-control" placeholder="Masukkan diskon" required value="{{ $promo->diskon }}">
                      </div>
                      <div style="width: 40%;">
                        <label for="persen">
                          <input type="radio" name="satuan" id="persen" value="persen" {{ $promo->satuan == "persen" ? "checked" : "" }}> %
                        </label>
                        <label for="nominal">
                          <input type="radio" name="satuan" id="nominal" value="nominal" {{ $promo->satuan == "nominal" ? "checked" : "" }}> Rp
                        </label>
                      </div>
                    </div>
                  </div>
                ...
                <div class="col-6">
                  <div class="form-group">
                    ...
                    <div class="d-flex align-items-center">
                      ...
                      @php
                        $awal = Carbon\Carbon::parse($promo->awal)->locale('id');
                        $awal->settings(['formatFunction' => 'translatedFormat']);
                        $tanggal_awal = $awal->format('Y-m-d');

                        $akhir = Carbon\Carbon::parse($promo->akhir)->locale('id');
                        $akhir->settings(['formatFunction' => 'translatedFormat']);
                        $tanggal_akhir = $akhir->format('Y-m-d');
                      @endphp

                      <div style="width: 45%;">
                        <input type="date" name="awal" id="awal" class="form-control" value="{{ $tanggal_awal }}" required>
                      </div>
                      <div class="text-center" style="width: 10%;">s/d</div>
                      <div style="width: 45%;">
                        <input type="date" name="akhir" id="akhir" class="form-control" value="{{ $tanggal_akhir }}" required>
                      </div>             
                    </div>
                  </div>
                </div>
                        ...
                        @foreach ($item->dataProduk as $key_produk => $item_produk)
                          <div>
                            <label for="produk_{{ $item->id }}_{{ $key_produk }}" style="font-weight: 300;">
                              <input type="checkbox" name="produk[]" id="produk_{{ $item->id }}_{{ $key_produk }}" class="produk_{{ $item->id }}" value="{{ $item_produk->id }}"
                                @foreach ($promo->dataPromoProduk as $item_promo_produk)
                                  {{ $item_promo_produk->produk_id == $item_produk->id ? "checked" : "" }}
                                @endforeach
                              > {{ $item_produk->nama }}                            
                            </label>
                          </div>
              ...
              <div class="mt-4">
                <button class="btn btn-primary" style="width: 200px;"><i class="fa fa-save"></i> Perbaharui</button>
              </div>
            </form>
          </div>
        </div>
